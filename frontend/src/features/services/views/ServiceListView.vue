  ...
  <main class="explorer">
      ...
      <div class="catalog-layout">
        ...
        <div class="catalog-main">
          ...
          <section id="all-services" class="panel service-results" aria-label="Resultados da pesquisa" :aria-busy="loading">
            <div class="results-toolbar"><div class="tabs" aria-label="Ordenar serviços"><button v-for="t in [{ id: 'all', name: 'Todos os serviços' }, { id: 'rated', name: 'Mais bem avaliados' }, { id: 'recent', name: 'Mais recentes' }]" :key="t.id" :class="{ active: tab === t.id }" :aria-pressed="tab === t.id" @click="tab = t.id">{{ t.name }}</button></div><label class="sort">Ordenar por<select v-model="sort"><option value="relevance">Relevância</option><option value="price">Menor preço</option><option value="rating">Avaliação</option></select></label></div>
